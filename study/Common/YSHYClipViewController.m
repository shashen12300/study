...
-(void)creatUI
{
    ...
    [self.view addSubview:_overView];
     _clipBtn = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    ...
    [_clipBtn addTarget:self action:@selector(clipBtnSelected:) forControlEvents:UIControlEventTouchUpInside];
    [_clipBtn setBackgroundColor:UIColorFromRGB(0xff6949)];
    [_clipBtn setFrame:CGRectMake(0, self.view.frame.size.height - 40, self.view.frame.size.width, 40)];
    [self.view addSubview:_clipBtn];
...
-(void)drawClipPath:(ClipType )clipType
{
    ...
    CGPoint center = self.view.center;
     self.circularFrame = CGRectMake(center.x - self.circulWidht, center.y - self.circulHeight, self.circulWidht * 2, self.circulHeight * 2);
    ...
    {
        //绘制圆形裁剪区域
        [path  appendPath:[UIBezierPath bezierPathWithArcCenter:self.view.center radius:self.circulWidht startAngle:0 endAngle:2*M_PI clockwise:NO]];
    }
    else
    {
        [path appendPath:[UIBezierPath bezierPathWithRect:CGRectMake(center.x - self.circulWidht, center.y - self.circulHeight, self.circulWidht * 2, self.circulHeight * 2)]];
    }
