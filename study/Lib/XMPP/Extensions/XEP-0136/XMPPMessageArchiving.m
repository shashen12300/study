...
- (void)xmppStream:(XMPPStream *)sender didReceiveMessage:(XMPPMessage *)message
{
	...
	if ([self shouldArchiveMessage:message outgoing:NO xmppStream:sender])
	{
        ...
        from = [[from componentsSeparatedByString:@"/"]lastObject];
        NSString*userName = [[NSUserDefaults standardUserDefaults]objectForKey:kXMPPJID];
        userName = [[userName componentsSeparatedByString:@"@"]firstObject];
        
        if (![from isEqualToString:userName]) {
