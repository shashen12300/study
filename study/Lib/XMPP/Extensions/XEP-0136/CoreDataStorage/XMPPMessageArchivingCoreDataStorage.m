...
- (XMPPMessageArchiving_Message_CoreDataObject *)composingMessageWithJid:(XMPPJID *)messageJid
                                                    ...
                                                    managedObjectContext:(NSManagedObjectContext *)moc messageId:(NSString*)messageId
{
	...
	[self scheduleBlock:^{
		...
		XMPPMessageArchiving_Message_CoreDataObject *archivedMessage =
		    [self composingMessageWithJid:messageJid
		                        streamJid:myJid
		                         outgoing:isOutgoing
                     managedObjectContext:moc messageId:message.messageId];
		...
		else
		{
            ...
            //添加消息类型字段
            archivedMessage.messageType = message.messageType;
            archivedMessage.messageId = message.messageId;
            archivedMessage.messageUrl = message.messageUrl;
            archivedMessage.messageThumbnail = message.messageThumbnail;
            archivedMessage.messageProgress = message.messageProgress;
            archivedMessage.messageLocalPath = message.messageLocalPath;
            archivedMessage.messageSize = message.messageSize;
            archivedMessage.messageDuration = message.messageDuration;
            archivedMessage.messageChatType = message.messageChatType;
			...
			if ([messageBody length] > 0)
			{
                ...
                //添加消息类型
				contact.messageType = message.messageType;
...
//            [[self appDelegate].chatDelegate getNewMessage:nil Message:nil];
        });
	}];
