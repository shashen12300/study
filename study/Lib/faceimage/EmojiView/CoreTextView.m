...
CGPoint CGPointFlippedMake(CGPoint point, CGRect bounds)
{
	return CGPointMake(point.x, CGRectGetMaxY(bounds) - point.y);
}

CGRect CGRectFlippedMake(CGRect rect, CGRect bounds)
{
                ...
                for (int j = 0; j < CFArrayGetCount(runs); j++) {
                    ...
                    if (num) {
                        ...
                        if (type == CustomGlyphAttributeURL) {// 如果是绘制链接
                            ...
                            NSString *imageName = [attDic objectForKey:kCustomGlyphAttributeImageViewName];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event 
{
    ...
    CGPoint mirrorPoint = CGPointFlippedMake(point, self.bounds);
