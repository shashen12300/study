...
NSString *const kCustomGlyphAttributeImageViewName = @"CustomGlyphAttributeImageViewName";
...
- (NSMutableAttributedString *)transformTextColor:(UIColor *)textColor
{
    ...
    for (NSTextCheckingResult *result in emojis)
    {
        ...
        NSMutableAttributedString *attSubStr = [self attributedStringWithString:subStr font:[UIFont systemFontOfSize:17] color:textColor];
        ...
        if (imageName)
        {
            ...
            metrics->ascent = 18; // 这2个来控制文字的上下位置
            metrics->descent = 6; //
            metrics->width = 24;  // 对应图片为24个点
            ...
            [newStr addAttribute:kCustomGlyphAttributeType 
                           ...
                           range:__range];
            [newStr addAttribute:kCustomGlyphAttributeImageViewName
                           value:imageName
                           range:__range];
        }
        else
...
- (NSMutableAttributedString *)attributedStringWithString:(NSString *)pureString font:(UIFont *)font color:(UIColor *)color
{
    NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:pureString];
    [self attributedString:attributedString
         setTextAttributes:@{NSFontAttributeName : font,
                             NSForegroundColorAttributeName : color}
                   atRange:[pureString rangeOfString:pureString]];
    
    return attributedString;
}

- (void)attributedString:(NSMutableAttributedString *)attributedString setTextAttributes:(NSDictionary *)attributes atRange:(NSRange)range
{
    for (NSString *name in attributes)
    {
        [attributedString addAttribute:name value:[attributes objectForKey:name] range:range];
    }
}
