    ...
    for(NSTextCheckingResult *match in resultArray) {
        ...
        for (int i = 0; i < face.count; i ++) {
            if ([face[i][@"cht"] isEqualToString:subStr]) {
                ...
                textAttachment.bounds = CGRectMake(0, offY, textAttachment.image.size.width, textAttachment.image.size.height);
...
{
    NSMutableAttributedString *attrStr = [self emotionStrWithString:string offY:offY];
    [attrStr addAttribute:NSFontAttributeName
                    value:[UIFont systemFontOfSize:font]
                    range:NSMakeRange(0, attrStr.length)];
    CGSize textSize = [attrStr boundingRectWithSize:CGSizeMake(maxWidth, CGFLOAT_MAX)
