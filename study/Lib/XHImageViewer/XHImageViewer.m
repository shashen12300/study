...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    self.pageLabel.text = [NSString stringWithFormat:@"%ld / %lu",[self pageIndex] + 1,(unsigned long)_imgViews.count];
}
...
- (void)showWithSelectedView:(UIImageView*)selectedView {
    ...
    // 添加的视图
    self.pageLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 80, Main_Width, 30)];
    [self addSubview:self.pageLabel];
    self.pageLabel.textColor = [UIColor whiteColor];
    self.pageLabel.textAlignment = NSTextAlignmentCenter;
    self.pageLabel.text = [NSString stringWithFormat:@"%ld / %lu",(long)currentPage + 1,(unsigned long)_imgViews.count];
...
- (void)didLongPress:(UILongPressGestureRecognizer *)sender {
    if(sender.state == UIGestureRecognizerStateBegan) {
        NSInteger now = [self pageIndex];
        UIImageView *imageV = (UIImageView *)_imgViews[now];
        if ([self.delegate respondsToSelector:@selector(imageViewer:withWillSaveImage:)]) {
            [self.delegate imageViewer:self withWillSaveImage:imageV.image];
        }
    }
}
