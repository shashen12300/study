...
#define kPadding 40
#define kPhotoViewTagOffset 800
...
//    CGFloat barY = self.view.frame.size.height - barHeight;
    ...
    _toolbar.frame = CGRectMake(0,  136, self.view.frame.size.width, barHeight);
...
- (void)addAlertControl
{
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:nil delegate:nil cancelButtonTitle:nil otherButtonTitles:@"保存到手机",@"转发给朋友",@"收藏",@"举报", nil];
    alert.delegate = self;
    [alert show];
    ...
    UIAlertAction *savePhont = [UIAlertAction actionWithTitle:@"保存到手机" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            MJPhoto *photo = _photos[_currentPhotoIndex];
            UIImageWriteToSavedPhotosAlbum(photo.image, self, @selector(image:didFinishSavingWithError:contextInfo:), nil);
        });
    ...
    UIAlertAction *transpond = [UIAlertAction actionWithTitle:@"转发给朋友" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
    ...
    [alertVC addAction:savePhont];
    [alertVC addAction:transpond];
