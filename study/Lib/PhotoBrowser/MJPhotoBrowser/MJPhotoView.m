...
- (void)showImage
{
    if (_photo.firstShow) { // 首次显示
        ...
        // 不是mp4，就马上开始下载
        if (![_photo.url.absoluteString hasSuffix:@"mp4"]) {
...
- (void)didLongTap:(UILongPressGestureRecognizer *)longTap
{
    if(longTap.state == UIGestureRecognizerStateEnded)
    {
        if (self.photoViewDelegate && [self.photoViewDelegate respondsToSelector:@selector(photoViewLongTap:)]) {
            [self.photoViewDelegate photoViewLongTap:self];
        }
        return;
        NSMutableDictionary *mutableDic = [[NSMutableDictionary alloc] init];
        [mutableDic setObject:@"alert" forKey:@"type"];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"didClickPartnerImage" object:mutableDic];
