...
//    _topBar.userInteractionEnabled = YES;
    ...
    //滑块的样式
    [_durationSlider setThumbImage:[UIImage imageNamed:@"Lecture_movieSlide"] forState:UIControlStateNormal];
    ...
    if (_style == ALMoviePlayerControlsStyleFullscreen || (_style == ALMoviePlayerControlsStyleDefault && _moviePlayer.isFullscreen)) {
        ...
        [_fullscreenButton setImage:[UIImage imageNamed:@"Lecture_movieEndFullscreen"] forState:UIControlStateNormal];
//        [_fullscreenButton setImage:[UIImage imageNamed:@"Lecture_movieEndFullscreen"] forState:UIControlStateSelected];
        ...
        [_playPauseButton setImage:[UIImage imageNamed:@"Lecture_moviePause"] forState:UIControlStateNormal];
        [_playPauseButton setImage:[UIImage imageNamed:@"Lecture_moviePlay"] forState:UIControlStateSelected];
        [_topBar addSubview:_playPauseButton];
        ...
        _fullscreenButton = [[ALButton alloc] init];
        [_fullscreenButton setImage:[UIImage imageNamed:@"Lecture_movieFullscreen"] forState:UIControlStateNormal];
        [_fullscreenButton addTarget:self action:@selector(fullscreenPressed:) forControlEvents:UIControlEventTouchUpInside];
        _fullscreenButton.delegate = self;
...
- (void)showControls:(void(^)(void))completion {
    if (!self.isShowing) {
        ...
        [UIView animateWithDuration:0.3 delay:0.0 options:0 animations:^{
            if (self.style == ALMoviePlayerControlsStyleFullscreen || (self.style == ALMoviePlayerControlsStyleDefault && self.moviePlayer.isFullscreen)) {
                self.topBar.alpha = 0.5f;
            }
            self.bottomBar.alpha = 0.5f;
...
- (void)layoutSubviews {
    ...
    CGFloat playWidth = 26.f;
    CGFloat playHeight = 28.f;
    ...
    if (self.style == ALMoviePlayerControlsStyleFullscreen || (self.style == ALMoviePlayerControlsStyleDefault && self.moviePlayer.isFullscreen)) {
        ...
        self.topBar.frame = CGRectMake(0, 20, self.frame.size.width, self.barHeight);
        ...
        //bottom bar
        self.bottomBar.frame = CGRectMake(0, self.frame.size.height - 30, self.frame.size.width, self.barHeight);
...
- (void)layoutSubviews {
    ...
    if (self.style == ALMoviePlayerControlsStyleFullscreen || (self.style == ALMoviePlayerControlsStyleDefault && self.moviePlayer.isFullscreen)) {
        ...
        // topBar的frame
        self.topBar.frame = CGRectMake(0, self.frame.size.height-30, self.frame.size.width, 30);
        
        self.playPauseButton.frame = CGRectMake(paddingFromBezel, self.barHeight/2 - fullscreenHeight/2, fullscreenWidth, fullscreenHeight);
        
        //Done
        self.fullscreenButton.frame = CGRectMake(self.topBar.frame.size.width - paddingFromBezel - 21, self.barHeight/2 - scaleHeight/2, fullscreenWidth, 30);
        //开始时间
        self.timeElapsedLabel.frame = CGRectMake(self.playPauseButton.frame.origin.x + self.fullscreenButton.frame.size.width + paddingBetweenButtons, 0, labelWidth, self.barHeight);
        ...
        //
        self.playPauseButton.frame = CGRectMake(paddingFromBezel, self.barHeight/2 - playHeight/2, playWidth, playHeight);
//        NSLog(@"self.playPauseButton.frame == %@",NSStringFromCGRect(self.playPauseButton.frame));
        
        self.timeElapsedLabel.frame = CGRectMake(self.playPauseButton.frame.origin.x + self.playPauseButton.frame.size.width + paddingBetweenButtons, 0, labelWidth, self.barHeight);
//        NSLog(@"self.timeElapsedLabel.frame == %@",NSStringFromCGRect(self.timeElapsedLabel.frame));
        //right side of bottom bar
        CGFloat fullscreenWidth = 28.f;
        CGFloat fullscreenHeight = fullscreenWidth;
        self.fullscreenButton.frame = CGRectMake(self.bottomBar.frame.size.width - paddingFromBezel - fullscreenWidth, self.barHeight/2 - fullscreenHeight/2, fullscreenWidth, fullscreenHeight);
