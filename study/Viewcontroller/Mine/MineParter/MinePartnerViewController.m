...
@interface MinePartnerViewController ()<UISearchBarDelegate, NSFetchedResultsControllerDelegate>
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.title = @"我的伙伴";
    [self setRightBarButton:@"Account_AddBank"];
    //从数据库读取数据
...
//    cell.cellLabel.text = user.nickname;
...
//- (void)getMinePartnerData
//{//通过xmppcoredata获取好友列表
//    NSManagedObjectContext *context = [[XMPPManager sharedManager] managedObjectContextRoster];
//    NSEntityDescription *entity = [NSEntityDescription entityForName:@"XMPPGroupCoreDataStorageObject" inManagedObjectContext:context];
//    NSSortDescriptor *sorted1= [NSSortDescriptor sortDescriptorWithKey:@"sectionNum" ascending:YES];
//    NSSortDescriptor *sorted2 = [NSSortDescriptor sortDescriptorWithKey:@"displayName" ascending:YES];
//    NSFetchRequest *request = [[NSFetchRequest alloc] init];
//    [request setEntity:entity];
//    [request setSortDescriptors:@[sorted1, sorted2]];
...
- (void)setRightBarButton:(NSString *)string{
    UIBarButtonItem *item = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:string] style:UIBarButtonItemStylePlain target:self action:@selector(addPartner)];
    self.navigationItem.rightBarButtonItem = item;
}

- (void)addPartner{
    AddPartnerViewController *add = [[AddPartnerViewController alloc] init];
